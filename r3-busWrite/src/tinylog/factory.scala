...
object factory {
  ...
  def buildBusWriteUnit4(r0: Bus, r1: Bus, r2: Bus, r3: Bus,
    ...
    var s0 = (r0 && !writeEnable) | (data &&(writeEnable && !writeTo(0) && !writeTo(1)))| (r0 && (writeEnable && !(!writeTo(0) && !writeTo(1))))
    var s1 = (r1 && !writeEnable) | (data &&(writeEnable && writeTo(0) && !writeTo(1))) | (r1 && (writeEnable && !(writeTo(0) && !writeTo(1))))
    var s2 = (r2 && !writeEnable) | (data &&(writeEnable && !writeTo(0) && writeTo(1))) | (r2 && (writeEnable && !(!writeTo(0) && writeTo(1))))
    var s3 = (r3 && !writeEnable) | (data &&(writeEnable && writeTo(0) && writeTo(1)))  | (r3 && (writeEnable && !(writeTo(0) && writeTo(1))))
    
    s0 = s0 && !reset          //If "reset" is true, then all bits in each of "s0", "s1", "s2", "s3" must be false.
    s1 = s1 && !reset
    s2 = s2 && !reset
    s3 = s3 && !reset
