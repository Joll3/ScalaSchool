...
package object futures {
  ...
  object ServicesF {
    ...
    def P(x: Future[Int], y: Future[Int]): Future[Int] = x.zip(y).map(c => c._1 + c._2)
    def Q(x: Future[Int], y: Future[Int]): Future[Int] = x.zip(y).map(c => c._1 * c._2)  
  }
  ...
  def zeroOnArithmeticException(x: Future[Int]): Future[Int] = {
    x.recoverWith{case e: ArithmeticException => zeroOnArithmeticException(Future(0))}
  }
