...
import o1.carsim.gui.GoogleDirections.DirectionsException
import o1.carsim.gui.GoogleDirections.Segment
...
class CarMap(tileSource: TileSource, center: Location, zoomLevel: Int) extends MapPanel(tileSource) {
  ...
  this.reactions += {
    case event: MousePressed =>
      ...
      if (this.clickedCar.isDefined && event.peer.getButton == 3) {
    ...
    case event: MouseReleased =>
      ...
      } else if (event.peer.getButton == 3) {
        this.showPopupMenuAt(releasePoint)
      }
      this.stopDragging()
    case MouseMoved(_, point, _) =>
      this.tooltip = this.carAt(point).map( _.marker.toString ).getOrElse(null)
  ...
  private val carMenu = new PopupMenu { 
    contents += new MenuItem(Action("Drive...") { 
      for {
        ...
        destination <- requestAnyLine("Destination, please: ", Some(thisMap)).filter( _.nonEmpty )
      } {
    ...
    contents += new MenuItem(Action("Fuel...")  { 
      for {
        ...
        amountWanted <- requestDouble("Add how many liters of fuel?", _ >= 0, "A non-negative number is required.", Some(this))
      } {
        thisMap.mostRecentCar = Some(carFueled)
        val gainedFuel = carFueled.fuel(amountWanted)
        if (gainedFuel == 0.0 && amountWanted > 0.0) {
          display("Could not add any fuel.", Some(thisMap))
        } else if (gainedFuel < amountWanted) {
          display("Could only add " + gainedFuel + " liters.", Some(thisMap))
        }
      }
    })
  ...
  private def createNewCar(pixel: Point) = {
    ...
    for {
      consumption <- requestDouble("Enter a fuel consumption rate (liters / 100 km) for the new car:", _ >= 0, "Non-negative number required.", Some(this))
      tankSize    <- requestDouble("Enter the size of its fuel tank in liters:",                       _ > 0, "Positive number required.", Some(this))
      fuelInTank  <- requestDouble("Enter amount of fuel in tank, in liters:",                         n => (n >= 0 && n <= tankSize), "A number between zero and the tank size, please.", Some(this))
  ...
  private def attemptDrive(carThread: CarThread, destination: String) = {
    ...
    Future(carThread.car.findRoute(destination)).onComplete {
      ...
      case Failure(directionsUnavailable: DirectionsException) => display(directionsUnavailable.message, Some(this))
...
private object CarMap {
  ...
  implicit def googleCoordsToMapViewerCoordinate(coords: GoogleDirections.Coords): Coordinate = new Coordinate(coords.lat, coords.lng)
