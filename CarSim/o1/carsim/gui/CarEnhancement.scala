...
import o1.carsim._
import GoogleDirections.Segment
...
class CarEnhancement(consumption: Double, tankSize: Double, fuelInTank: Double, initialLocation: Location) {
  ...
  def findRoute(destination: String) = {
    ...
    val segments = GoogleDirections.findRoute(this.car.location.toString, destination).toIndexedSeq
...
private object CarEnhancement {
  ...
  implicit def googleCoordsToLocation(coords: GoogleDirections.Coords): Location = Location(coords.lat, coords.lng) 
