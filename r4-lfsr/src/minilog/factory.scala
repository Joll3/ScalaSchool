...
object factory {
  ...
  def buildRotator(state: Bus): Bus = {
    ...
    for(i <- 0 until state.length - 1){
      current(i).buildFeedback(current(i+1))
    }
    current(state.length-1).buildFeedback(current(0))
  ...
  def buildLFSR(taps: List[Int], state: Bus): Bus = {
    ...
    state(state.length -1).buildFeedback(taps.map(x => state(x)).foldLeft(state(0))(_^^_))
