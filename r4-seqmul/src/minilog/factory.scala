...
object factory {
  ...
  def buildSequentialMultiplier(aa: Bus, bb: Bus, loadEnable: Gate): (Gate, Bus) = {
    ...
    val operand1 = loadEnable.host.inputs(aa.length+bb.length)
    val operand2 = loadEnable.host.inputs(bb.length)
    val result = loadEnable.host.inputs(aa.length + bb.length)
    var ready = (!operand2.reduce(_||_)) && (!loadEnable)
    
    val a = aa ++ loadEnable.host.falses(bb.length)
  
    operand1.buildFeedback((a && loadEnable) | ((loadEnable.host.falses(1) ++ operand1.dropRight(1)) && !loadEnable))
    operand2.buildFeedback((bb && loadEnable) | ((operand2.drop(1) ++ loadEnable.host.falses(1)) && !loadEnable))
    
    result.buildFeedback(buildAdder(result,(operand1 && operand2(0))) && !loadEnable)
