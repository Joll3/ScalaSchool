...
package object sequences {
  ...
  def sumOfSquaresOfOdd(in: Seq[Int]): Int =
    in.filter(_%2!=0)         // (1)  [when making a cascade it is convenient ..]
      ...
      .reduceLeft(_+_)         // (3)  [.. lines to ease readability]
  ...
  def minOfSquaresAtLeast17OfPositive(in: Seq[Int]): Int = in.filter(_>0).map(x=>x*x).filter(_>=17).min
  ...
  def sumOfProductsOfPairs(in: Seq[(Int,Int)]): Int =
    in.map((x)=>x._1*x._2).reduceLeft(_+_)
  ...
  def innerProductOfTwoDDimVectors(a: Seq[Int], b: Seq[Int]): Int =
    (a zip b).map((x)=>x._1*x._2).reduceLeft(_+_)
  ...
  def squareOfDDimEuclideanDistance(a: Seq[Int], b: Seq[Int]): Int =
    (a zip b).map((x)=>x._1-x._2).map((x)=>x*x).reduceLeft(_+_)
  ...
  def movingSum(in: Seq[Int], w: Int): Seq[Int] = {
    ...
    ((in.scan(0)(_+_).drop(w)) zip (in.scan(0)(_+_).dropRight(w))).map((x)=>x._1-x._2)
  }
}
