...
import o1.auctionhouse.AuctionHouse
...
object TestApp extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    contents = new GridBagPanel {
      layout += statusLine -> new Constraints(0, 0, 1, 1, 1, 0, NorthWest.id, Fill.None.id, new Insets(5, 5,  5, 5), 0, 0)
      layout += buttonRow  -> new Constraints(0, 1, 1, 1, 1, 0, NorthWest.id, Fill.None.id, new Insets(0, 0,  0, 0), 0, 0)
      layout += itemTabs   -> new Constraints(0, 2, 1, 1, 1, 0, NorthWest.id, Fill.Both.id, new Insets(0, 0,  0, 0), 0, 0)
    }
    ...
    def descriptionParam() = {
      requestNonEmptyLine("Item description: ", "Please enter a non-empty description.", Some(buttonRow))
    }

    def intParam(prompt: String) = {
      requestInt(prompt + ": ", _ >= 0, "Please enter a reasonable integer value.", Some(buttonRow)).map( new Integer(_) )
    }
    
    abstract class ItemPanel[+SaleType <: ItemForSale](val item: SaleType) extends GridBagPanel { 
      ...
      layout += buyAction -> new Constraints(0, 5, 3, 1, 3, 0, NorthWest.id, Fill.None.id, new Insets(5,  20, 10, 0), 0, 0)
