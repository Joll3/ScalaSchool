...
object factory {
  ...
  def buildOneWordMemory(readEnable: Gate, writeEnable: Gate, data: Bus): Bus = {
    ...
    //println(state(0).value,state(1).value,state(2).value,state(3).value)// new bus with length == data.length and full of falses
    

      state.buildFeedback((data && writeEnable) | (state && !writeEnable))
      //println(state(i).value + " " + i)
       //new bus with length == 4 and all values false
     (state && readEnable)
  ...
  def buildMemory(readEnable: Gate, writeEnable: Gate, address: Bus, data: Bus): Bus = {
    ...
    require(data.length > 0, "Data bus must have positive width.")
    var bussit = Seq[Bus]()             // Bus w len = data.length full of false
    var osoite = buildDecoder(address)
    
    for(i <- 0 until osoite.length){
      bussit = bussit :+ buildOneWordMemory(readEnable, writeEnable && (osoite(i)), data)
    }
    buildBusSelector(bussit,osoite)
  }
}
