...
abstract class Scenario(val name: String) {
  ...
  protected def requestParameter(prompt: String, min: Int, locator: Component) = {
    requestInt(prompt, _ >= min, "Please enter an integer no lower than " + min + ".", Some(locator))
  }
...
object Scenario {
  ...
  object Bar extends MapScenario("Bar") {
    ...
    protected def addBots(world: RobotWorld) = {
      ...
      for (loc <- flyLocs) {
        flyCount += 1
        val fly = world.addRobot(loc, Direction.ValuesClockwise(flyCount % 4))
  ...
  abstract class RandomScenario(name: String) extends Scenario(name) {
    ...
    def makeDirection() = {
      Direction.ValuesClockwise(util.Random.nextInt(Direction.ValuesClockwise.size))
    }
