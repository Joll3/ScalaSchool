...
class RobotWindow extends MainFrame {  
  ...
  protected val nextFullTurnButton = new Button {
    action = Action("Full turn") { worldView.nextTurns(1) }
    preferredSize = nextRobotTurnButton.preferredSize
  }
  val buttonPanel = new FlowPanel(nextRobotTurnButton, nextFullTurnButton)
 
  this.contents = new FlowPanel(new GridBagPanel {
    layout += worldPane   -> new Constraints(0, 0, 2, 1, 1, 1, NorthWest.id, Fill.Both.id, new Insets(0, 0, 0, 0), 0, 0)
    layout += buttonPanel -> new Constraints(0, 1, 1, 1, 1, 0, NorthEast.id, Fill.None.id, new Insets(0, 0, 0, 0), 0, 0)
  } )
    
  protected def createView(world: RobotWorld): RobotsView = new RobotsView(this, world)
    
  def focusButton = this.nextFullTurnButton 
  ...
  protected def updateButtons(nextRobot: Option[RobotBody]) = {
    ...
    this.nextFullTurnButton.enabled  = nextRobot.isDefined
  ...
  protected class RobotsView(val parent: RobotWindow, val world: RobotWorld) extends BasicGridView[RobotWorld, Square](world, RobotsView.MaxSquareSize) {
    ...
    def nextTurns(howMany: Int): Unit = {
      for (repeat <- 1 to howMany) {
        this.world.nextFullTurn()
      }
      this.update()
    }
    ...
    protected class SquarePopup extends Popup {
      ...
      abstract class AbstractAddRobotItem(name: String) extends PopupAction(name) {
        ...
        def requestName() = {
          requestAnyLine("Robot name:", Some(view))
        }

        def requestFacing() = {
          val dirs         = Direction.ValuesClockwise
          ...
          for (choice <- requestChoice("Initial facing:", choices, Some(view))) yield (dirMap + randomChoice)(choice)
        }
      ...
      object AddStaggerbotItem extends AddBasicRobotItem("Add staggerbot", Staggerbot) {
        def requestParameters(name: String, body: RobotBody) = {
          for (seed <- requestAnyInt("Random seed:", "Please enter an integer.", Some(view)))
      ...
      object AddLovebotItem extends AddBasicRobotItem("Add lovebot", Lovebot) {
        ...
        def requestBeloved(self: RobotBody) = {
          ...
          if (options.size == 0) {
            ...
            val chosenBrain = requestChoice("Choose the beloved:", options, Some(view))
      ...
      val fixItem = new RobotAction("Fix", robot => !robot.isWorkingRight && robot.brain.isDefined )( _.fix() )
      val destroyItem = new RobotAction("Destroy", _.isWorkingRight )( _.destroy() )
    ...
    val directionPics = Direction.ValuesClockwise.mapify(identity)( dir => this.getPic("arrow_" + dir, true) )
    ...
    def getPic(name: String, fullSize: Boolean): BufferedImage = {
      this.getPic(ImageIO.read(this.getClass.getResource("/pictures/" + name.toLowerCase + ".png")), fullSize)
    }

    def getPic(image: BufferedImage, fullSize: Boolean) = {
    ...
    def visualsFor(possiblyInitializedSquare: Option[Square]) = {
      possiblyInitializedSquare.map( this.visualsFor(_) ).getOrElse(Array.empty)
    }
    
    private def visualsFor(square: Square) = {
      val terrainPic   = if (square.isUnpassable) this.wallPic else this.floorPic
      val statusPic    = if (square.robot.exists( !_.isWorkingRight )) this.brokenPic else null
      val robotPic     = square.robot.map( this.robotPic(_) ).getOrElse(null)
      val directionPic = square.robot.map( robot => this.directionPics(robot.facing) ).getOrElse(null)
      Array(terrainPic, robotPic, directionPic, statusPic)
    }
