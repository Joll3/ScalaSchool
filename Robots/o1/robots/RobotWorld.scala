...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight +2) { // TODO: add "extends ..." 
  ...
  private var turn = 0 // counter for turns taken not affected by nextFullTurn
  
  for (x <- 0 until floorWidth + 2; y <- 0 until floorHeight + 2) { // Note to students: this is one way of looping through both x and y coordinates. You could achieve the same by using two separate for loops and nesting the "y loop" within the "x loop".
    if (x == 0 || x == floorWidth + 1 || y == 0 || y == floorHeight + 1) this.update(new Coords(x,y), Wall)
    else this.update(new Coords(x,y),new Floor)// TODO: This adds walls everywhere. Replace with a working implementation.  
  }
  ...
  def addRobot(initialLocation: Coords, initialFacing: Direction) = {
    ...
    robots += newBot
    this(initialLocation).addRobot(newBot)
    newBot
    // TODO: replace this line with a working implementation
  }
  ...
  def addWall(location: Coords) = {
    this.update(location, Wall)// TODO: implementation missing
  }
  ...
  /** Returns the robot whose turn it is to act. That is, returns the robot who will be the 
    * next one to act when [[nextTurn]] or [[nextFullTurn]] is called. 
    *
    ...
    */
  def nextRobot = {
    if(!robots.isEmpty)Some(robots(turn)) else None
