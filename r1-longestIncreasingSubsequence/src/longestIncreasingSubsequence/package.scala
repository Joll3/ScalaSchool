...
package object longestIncreasingSubsequence {
  ...
  def longestIncreasingSubsequence(s: Seq[Int]): Seq[Int] = {
    ...
    for(i <- 0 until s.length){
      ...
      while(lo <= hi){
        var mid = (((lo+hi)/2).toDouble.ceil.toInt)
        if(s(m(mid))< s(i)){
          lo = mid + 1
        }
         else hi = mid - 1
    ...
    return seq
    /**def ceilIndex(increasing: Seq[Int],total: Seq[Int], left: Int, right: Int, key: Int): Int = {
      var r: Int = right
      var l: Int = left  
      
      while ((r - l) > 1){
        var m:Int = (left+(right-1))/2
        if(increasing(total(m)) >= key) r = m else l = m
      }
      r
    }
    
    def LongestIncSubSeq(input: Seq[Int], size: Int): Seq[Int] = {
      var length = 1
      var tailIndices = Array.fill(size)(0)
      var prevIndices = Array.fill(size +1)(-1)
      var output = Seq[Int]()
      ...
      for(i <- 1 until (size)){
        ...
        else if(input(i) > input(tailIndices(length -1))){
          prevIndices(i) = tailIndices(length - 1)
          tailIndices(length) = i
          length += 1
        }
        else {
          var position = ceilIndex(input, tailIndices, -1, length - 1, input(i))
          prevIndices(i) = tailIndices(position -1)
          tailIndices(position) = i
        }
      }
      
      for (i <- 0 until tailIndices.length){
        output :+ input(i)
      }
    ...
    return LongestIncSubSeq(s, s.length)*/
