...
object factory {
  ...
  def buildUnsignedLess(bus1: Bus, bus2: Bus): Gate = {
    ...
    def buildGategi(b1i: Gate, b2i: Gate, giminus1: Gate): Gate = {
      var gi = (!b1i && !b2i && giminus1) || (!b1i && b2i && !giminus1) || (!b1i && b2i && giminus1) || (b1i && b2i && giminus1)
      gi
    }
  ...
  def buildIncrementer(bus: Bus): Bus = {
    ...
    var add = Bus.trues(1) ++ Bus.falses(bus.length-1)
    return buildAdder(add,bus)
  }
  ...
  def buildCountTrueBits(bus: Bus): Bus = {
    ...
    var current = Bus.trues(1) ++ Bus.falses(log2ceil(bus.length)-1)
    var palauta = Bus.falses(log2ceil(bus.length))
    
    for(i <- 0 until bus.length){
      var testaa = current && bus(i)
      palauta = buildAdder(testaa,palauta)
  ...
  def buildMajorityTester(bus: Bus): Gate = {
    ...
    var truet = buildCountTrueBits(bus)
    var falset = buildCountTrueBits(~bus)             //TODO? length might not be % 2
    buildUnsignedLess(falset,truet)                                        
  }
  ...
  def buildMultiplier(bus1: Bus, bus2: Bus): Bus = {
    ...
    var palauta = Bus.falses(bus1.length + bus2.length)
    for(i<- 0 until bus2.length){
      palauta = buildAdder(palauta, Bus.falses(i) ++ bus1 ++ Bus.falses(bus2.length-i)&& bus2(i))
    }
    palauta
  }
}
