...
abstract class BasicGridView[GridType <: Grid[Element], Element](val grid: GridType, maxSquareSize: Int) extends Component {
  ...
  this.reactions += {
    ...
    case MouseMoved(_, point, _) =>
      this.tooltip = this.elementAt(point).flatMap( this.tooltipFor(_) ).getOrElse(null)
  }
  ...
  def visualsFor(element: Option[Element]): Array[BufferedImage]    
  ...
  private class Cell(row: Int, column: Int)  {
    ...
    def updatePics() = {
      val newPics = view.visualsFor(this.element)
