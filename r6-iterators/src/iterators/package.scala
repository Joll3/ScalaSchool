...
package object iterators {
   ...
   * (Strictly speaking, 1 is not considered prime in most of the modern definitions as including it causes problems in some 
   * other definitions but let's consider it to be a prime in this exercise.)
   */
  def primesIterator: Iterator[Int] = new Iterator[Int] {
     ...
     */
    def next(): Int = {                                                            //otettu mallia internetistä
...
//      }
//      
//      val palauta = calculatePrimes(state*2).takeRight(1)
//      if((state == 1)|| (state == 2)){
//        state = state + 1
//        }
//      else state = palauta(0)
//      state
//////////////////////////////
//      val end = state*2
//      val odds = Stream.from(3, 2).takeWhile(_ <= Math.sqrt(end).toInt)
...
//      Stream.from(3, 2).takeWhile(_ <= end).diff(composites).toList
  ...
  def blockingRandomIterator[T](s: Set[T], blockingTime: Int, seed: Int = 2105): Iterator[T] = new Iterator[T] {
    ...
    var blocked = collection.mutable.Map[T, Int]()
    var notblocked = List[T]()
    ...
    def next(): T = {
      blocked = blocked.map(f => (f._1, f._2-1))
      val pickfrom = s -- blocked.filter(_._2>=0).keys
      //println(pickfrom, blocked.filter(_._2>=0).keys)
      val palauta = pickfrom.toVector(r.nextInt(pickfrom.size))
      blocked(palauta) = blockingTime
      palauta
    }
  }
}
