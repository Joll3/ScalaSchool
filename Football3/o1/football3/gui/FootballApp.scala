...
object FootballApp extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    class SeasonPanel(val owner: Window) extends GridBagPanel {
      ...
      val stats = new GridBagPanel {
        layout += countStat   -> new Constraints(0, 0, 1, 1, 1, 0, NorthWest.id, Fill.None.id, new Insets(0, 5, 3, 5), 0, 0)
        layout += latestStat  -> new Constraints(1, 0, 1, 1, 1, 0, NorthWest.id, Fill.None.id, new Insets(0, 5, 3, 5), 0, 0)
        layout += highestStat -> new Constraints(1, 1, 1, 1, 1, 0, NorthWest.id, Fill.None.id, new Insets(0, 5, 3, 5), 0, 0)
      }
      ...
      layout += playedLabel   -> new Constraints(0, 0, 1, 1, 0, 0, NorthWest.id, Fill.None.id,     new Insets(5, 5, 3, 5), 0, 0)
      layout += matchScroller -> new Constraints(0, 1, 1, 1, 0, 1, NorthWest.id, Fill.Vertical.id, new Insets(0, 5, 3, 5), 0, 0)
      layout += startButton   -> new Constraints(0, 2, 1, 1, 0, 0, NorthWest.id, Fill.None.id,     new Insets(0, 5, 3, 5), 0, 0)
      layout += matchView     -> new Constraints(1, 0, 1, 2, 1, 1, NorthWest.id, Fill.Both.id,     new Insets(5, 5, 3, 5), 0, 0)
      layout += stats         -> new Constraints(1, 2, 1, 1, 1, 0, NorthWest.id, Fill.None.id,     new Insets(5, 5, 3, 0), 0, 0)

      def addMatch() = {
        for {
          home <- requestChoice("Home team:", ExampleLeague.Clubs,                        Some(this))  
          away <- requestChoice("Away team:", ExampleLeague.Clubs.filterNot( _ == home ), Some(this))  
        } {
