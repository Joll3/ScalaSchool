...
abstract class Gate() {
   ...
   * */
  def inputElementSupport: Set[InputElement] = this.support.filter { x => x.isInstanceOf[InputElement] }.map { x => x.asInstanceOf[InputElement] }
}
...
class OrGate(in1: Gate, in2: Gate) extends Gate() {
  ...
  def depth: Int = math.max(in1.depth, in2.depth) + 1
...
class MajorityGate(in1: Gate, in2: Gate, in3: Gate) extends Gate() {
  def value = if((List(in1.value,in2.value,in3.value).count(_ == true)) >= 2) true else false
  def depth: Int = math.max(in1.depth, math.max(in2.depth,in3.depth)) + 1
