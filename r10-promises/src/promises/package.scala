...
package object promises {
   ...
   * 
   * metodia kutsutaan monta kertaa/peräkkäin
   *
   */
  ...
  def buildPlusZeroFailF(): (Promise[Int], Promise[Int], Future[Int]) = {
    ...
    (p, q, f.recoverWith{case _ => Future(0)})
  }
   ...
   * "onComplete", "foreach", and "failed" methods in following doc, also TRY SUCCESS possible instead of onComplete
  ...
  def fastestSuccessfulOfTwo(a: Future[Int], b: Future[Int]): Future[Int] = {
    val p = Promise[Int]
    a.foreach(f => p.trySuccess(f))
    b.foreach(f => p.trySuccess(f))
    a.failed.foreach(f => b.failed.foreach(ff => p.tryFailure(ff)))
    p.future
  }
  ...
  def buildTimesFast0F(): (Promise[Int], Promise[Int], Future[Int]) = {
    ...
    a.future.foreach(f => if(f == 0) p.trySuccess(0) 
        else p.tryCompleteWith(for {x <- a.future; y <- b.future} yield x * y))
    b.future.foreach(f => if(f == 0) p.trySuccess(0) 
        else  p.tryCompleteWith(for {x <- a.future; y <- b.future} yield x * y ))
    a.future.failed.foreach(f => b.future.failed.foreach(ff => p.tryFailure(ff)))
