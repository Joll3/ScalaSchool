...
package object base64 {
  ...
  def to24Bits(p: Int, q: Int, r: Int): Int = {
      var output = 0xFF&r
      //println("alku" + output.toBinaryString)
      output = ((0xFF&q)<<8)|output
      //println(output.toBinaryString)
      output = ((0xFF&p)<<16)|output
      //println(output.toBinaryString)
      output
    } 
  ...
  def to6BitWords(w: Int): Array[Int] = {
    var output = Array[Int](0,0,0,0)
    output(0)=(0xFC0000&w)>>18
    output(1)=(0x03F000&w)>>12
    output(2)=(0x000FC0&w)>>6
    output(3)=(0x00003F&w)
    output
  }
  ...
  def restrictedEncode(b: Array[Byte]): String = {
    ...
    var output = b.grouped(3).toArray
      .map(x=>to24Bits(x(0),x(1),x(2)))
      .map(x=>to6BitWords(x)).flatten
      .map(x=>B64(x)).foldLeft("")(_+_)
    output
  ...
  def encode(b: Array[Byte]): String = {
    ...
    if(b.length%3==0){
      restrictedEncode(b)
        ...
        var output = b :+ nolla :+nolla
        restrictedEncode(output).dropRight(2) + "=" +"="
        }
      else{
        var output = b :+ nolla
        restrictedEncode(output).dropRight(1)+ "="
      }
