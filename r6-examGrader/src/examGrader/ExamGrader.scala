...
class ExamGrader(val studentDB: StudentDB,
  ...
  lazy val graded: Set[StudentId] = {
    examPoints.keys.filter(homeworkDone.contains(_)).toSet
  ...
  def pointsToGrade(n: Int): Int = {
    if (((gradeLimits.indexWhere(_ > n)) min 5) == -1) 5 else (gradeLimits.indexWhere(_ > n) min 5)  //test this
  }
  /**
  ...
  def points(id: StudentId): Int = {
    ...
    //if(bonusPoints.keySet.contains(_ == id)) bonusPoints(id) + examPoints(id).sum
    bonusPoints.getOrElse(id, 0) + examPoints(id).sum
   ...
   */
  lazy val grades: Map[StudentId, Int] = graded.map(id => (id, pointsToGrade(points(id)))).toMap
   ...
   */
  def bestStudents(n: Int): Seq[StudentId] = graded.map(id => (id, points(id))).toList.sortBy(-_._2).take(n).map(x => x._1)
  ...
  def studentsByGrade: Map[Int, Set[StudentId]] = {
    val tulos = (0 to 5).map(x => (x, grades.filter(_._2 == x).map(_._1).toSet)).toMap //TODO sijoita graded
//    val tulos = Map(0 -> grades.filter(_._2 == 0))
//    for (i <- 1 to 5){
//    val i = tulos + 
    tulos
    }
  /**
   * The set of study programmes whose students took the exam and whose exam was graded. to set lopussa poistaa duplikaatit
   */
  def gradedProgrammes: Set[StudyProgramme] = grades.keySet.map(x => this.studentDB.programmeOf(x)).toSet
  ...
  def gradesByProgramme: Map[StudyProgramme, Map[StudentId, Int]] = {
    gradedProgrammes.map(x => (x, (graded.filter(y => this.studentDB.programmeOf(y) == x).map(id => (id, pointsToGrade(points(id)))).toMap))).toMap
    //gradedProgrammes.map(x => (x, grades.filter((y,z) => this.studentDB.programmeOf(y) == x)))
  }
}
